...
for w in range(0,2):
    ...
    if c==0:
        ...
        errfilt = yerru<0.1
        pcirdark = ax.errorbar(ydata[errfilt],xdata[errfilt],xerr=np.array([yerrd[errfilt],yerru[errfilt]]),color='indigo',marker=mark2,ms=4,lw=0.5,ls='None',zorder=11,label='Significant H$\\alpha$ detection (error <0.1 dex)')
    else:
...
if ssfr:
    ...
    else: fig.savefig(figout + 're_sSFR_V2.pdf')
