...
fig,axarr = plt.subplots(gridsize,gridsize,figsize = (8,7))
                ...
                if np.count_nonzero(~np.isnan(specline)):
                    try:
                        ...
                        def mkplot(plt10,plt1,plt10b,plt1b,gridsize):
                            ...
                            ax0.plot(waveline,modelline*scale3,color='red',label='Model',ls='-',alpha=0.5,zorder=100)
                        ...
                        if not HaNII:
                            ax0.text(0.02,0.95,'Redshift:   ' + str(round(zcc,4)),fontsize = textfont, transform=ax0.transAxes)
                            ax0.text(0.02,0.90,'Mean:       ' + str(round(mu3,2)),fontsize = textfont, transform=ax0.transAxes)      
                            ax0.text(0.02,0.85,'Sigma:      ' + str(round(stddev3,2)),fontsize = textfont, transform=ax0.transAxes)
                            ...
                            ax0.text(0.02,0.70,'S/N:          ' + str(round(usigrat,3)),fontsize = textfont, transform=ax0.transAxes)
                        else:
                        ...
                        if not HaNII:
                            ax0.plot(dropwaveline,gausscurve3,color='black',label='Gaussian fit',alpha=0.75)
                        else:
...
fig.savefig(figout + 'EmissionFit_Thesis.pdf')
plt.close(fig)
