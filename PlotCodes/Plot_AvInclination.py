...
for w in [0]:
    ...
    if c==0:
        ax.set_ylabel('b/a',fontsize = axisfont)
    if c==3:
    ...
    if c in [0,3]:
        ...
        ax.errorbar(med1,loc1/2,xerr=(emed1/len(fluxdata[filters][mr1])),color='red',marker='o',ms=ms,lw=lwbw,ls='None')
        ax.errorbar(med2,(loc1+loc2)/2,xerr=(emed2/len(fluxdata[filters][mr2])),color='red',marker='o',ms=ms,lw=lwbw,ls='None')
        ax.errorbar(med3,(loc2+loc3)/2,xerr=(emed3/len(fluxdata[filters][mr3])),color='red',marker='o',ms=ms,lw=lwbw,ls='None')
        ax.errorbar(med4,(1+loc3)/2,xerr=(emed4/len(fluxdata[filters][mr4])),color='red',marker='o',ms=ms,lw=lwbw,ls='None')
...
fig.tight_layout()
if combinemass: fig.savefig(figout + 'Av_inclination_combmass.pdf')
else:fig.savefig(figout + 'Av_inclination.pdf')
plt.close(fig)
